...
export class AMSCardEditor extends LitElement {
  ...
  async setConfig(config) {
    ...
    const bambu_devices = device_list.filter((obj) =>
      obj.identifiers[0].includes("bambu_lab"),
    );
    console.log("bambu_devices", bambu_devices);
    this._devices = bambu_devices.filter((obj) =>
      ams_models.includes(obj.model),
    );
    this._config = config;
  }
            ...
            <form class="table">
                <div class="row">
                    ...
                    <input
                            ...
                            class="value cell" id="header" value="${
                              this._config.header
                            }"></input>
                </div>
                <div class="row">
                    <label class="label cell" for="style">Card:</label>
                    <select name="device" id="style" @change="${
                      this.handleChangedEvent
                    }"
                            class="value cell" id="style" value="${
                              this._config.style
                            }">
                ...
                <div class="row">
                    ...
                    <select name="device" id="device" @change="${
                      this.handleChangedEvent
                    }"
                            class="value cell" id="device" value="${
                        ...
                        ${this._devices.map(
                          (d) =>
                            html`<option
                              value="${d.id}"
                              selected=${this._config.device_id === d.id}
                            >
                              ${d.name}
                            </option>`,
                        )}
